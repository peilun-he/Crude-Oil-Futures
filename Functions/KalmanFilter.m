function [logL, Q, table_at_filter, table_at_prediction, table_at_smoother, ft] = KalmanFilter(par, yt, T, dt, smoothing, seasonality)
...
%   seasonality: a boolean variable indcicate if seasonal effect is
%   modelled
% Outputs: 
...
% Seasonal effect: f_t = b*t + beta*cos(2*pi*t*dt) + eta*sin(2*pi*t*dt)
seasonal_time = 1: nT; % t
ft = b*seasonal_time + beta*cos(2*pi*seasonal_time*dt) + eta*sin(2*pi*seasonal_time*dt); 
